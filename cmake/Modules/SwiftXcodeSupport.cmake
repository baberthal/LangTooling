...
function(escape_path_for_xcode config path result_var_name)
  ...
  get_effective_platform_for_triple("${SLT_HOST_TRIPLE}" SLT_EFFECTIVE_PLATFORM_NAME)
  ...
  string(REPLACE "$(EFFECTIVE_PLATFORM_NAME)" "${SLT_EFFECTIVE_PLATFORM_NAME}" result "${result}")
...
function(fixup_imported_target_property_for_xcode target property llvm_build_type)
  ...
  foreach(build_type ${CMAKE_CONFIGURATION_TYPES})
    ...
    set(SLT_BUILD_PROPERTY_NAME "${property}_${build_type_upper}")
    set_target_properties(${target} PROPERTIES "${SLT_BUILD_PROPERTY_NAME}" "${PROP_VALUE}")
  endforeach()
endfunction()
...
# if we compile LLVM-Release and SLT-Debug, SLT is going to look in the
...
macro(slt_common_xcode_cxx_config)
